...
function Reader() {
  ...
  useEffect(() => {
    ...
    // Hide pop-up when clicking elsewhere
    ...
    const fetchChapter = async () => {
      ...
      try {
        ...
        if (!chapter || !chapter.file) {
          throw new Error('Chapter file path is missing')
        }
  ...
  if (bookError) return <div className="reader-state">Error loading book: {bookError}</div>
  if (!book) return <div className="reader-state">Book not found.</div>
  ...
  return (
    <div className={`reader reader--theme-${theme} ${isFullWidth ? 'reader--full-width' : ''}`} onClick={toggleHeader}>
      ...
      <aside className={`reader__sidebar ${isSidebarOpen ? 'reader__sidebar--open' : ''}`} onClick={handleSettingClick}>
        ...
        <ul className="reader__chapter-list">
          {chapters.map((ch, index) => (
            <li 
              key={index} 
              className={`reader__chapter-item ${index === currentChapterIndex ? 'reader__chapter-item--active' : ''}`}
      ...
      <aside className={`reader__sidebar reader__sidebar--notes ${isNotesOpen ? 'reader__sidebar--open' : ''}`} onClick={handleSettingClick}>
        ...
        <div className="reader__notes-list">
          {bookNotes.length === 0 ? (
            <p className="reader__no-notes">No notes for this book yet.</p>
          ) : (
            bookNotes.map(note => (
              <div 
                key={note.id} 
                className="reader__note-item" 
                onClick={() => handleNoteClick(note.chapterOrder)}
                style={{ cursor: 'pointer' }}
              >
                <div className="reader__note-meta">
                  <span>Ch. {note.chapterOrder}</span>
                  <span>{note.timestamp}</span>
                </div>
                <p className="reader__note-text" style={{ fontSize: `${Math.max(0.9, fontSize * 0.8)}rem` }}>{note.text}</p>
                <button 
                  className="reader__note-delete" 
                  onClick={(e) => {
                    e.stopPropagation() // Prevent clicking note to navigate
                    deleteNote(note.id)
                  }}
                >
                  Delete
      ...
      <main className="reader__content" style={{ fontSize: `${fontSize}rem` }}>
        {isFetchingChapter ? (
          <div className="reader-state">Loading chapter...</div>
        ) : chapterError ? (
          <div className="reader-state error">Error loading chapter: {chapterError}</div>
        ) : (
          <article className="reader__markdown">
      ...
      <div className={`reader__controls-bar ${isHeaderVisible ? 'reader__controls-bar--visible' : ''}`} onClick={handleSettingClick}>
        ...
        <div className="reader__controls-inner">
          ...
          <div className="reader__settings-group reader__settings-group--font">
            <button className="reader__setting-btn" onClick={() => setFontSize(prev => Math.max(0.8, prev - 0.1))}>A-</button>
            <button className="reader__setting-btn" onClick={() => setFontSize(prev => Math.min(2.5, prev + 0.1))}>A+</button>
          </div>

          <div className="reader__controls-divider"></div>

          <div className="reader__settings-group reader__settings-group--theme">
            <button className={`reader__theme-btn reader__theme-btn--default ${theme === 'default' ? 'active' : ''}`} onClick={() => setTheme('default')} title="Default"></button>
            <button className={`reader__theme-btn reader__theme-btn--dark ${theme === 'dark' ? 'active' : ''}`} onClick={() => setTheme('dark')} title="Dark"></button>
            <button className={`reader__theme-btn reader__theme-btn--light ${theme === 'light' ? 'active' : ''}`} onClick={() => setTheme('light')} title="Light"></button>
          </div>

          <div className="reader__controls-divider reader__desktop-only"></div>

          <button 
            className="reader__setting-btn reader__desktop-only" 
          ...
          <button className="reader__setting-btn reader__setting-btn--notes" onClick={() => setIsNotesOpen(true)}>
            Notes
          </button>

          <div className="reader__controls-divider"></div>
          
          <button className="reader__setting-btn reader__setting-btn--chapters" onClick={() => setIsSidebarOpen(true)}>
            Chapters
          </button>
